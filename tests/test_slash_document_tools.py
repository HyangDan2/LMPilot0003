...
class SlashDocumentToolsTests(unittest.TestCase):
    ...
    def test_build_doc_map_auto_saves_from_context(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue((root / "llm_result" / "document_pipeline" / "report_pptx" / "document_map.json").exists())
    ...
    def test_generate_markdown_auto_saves_report(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            report_path = root / "llm_result" / "document_pipeline" / "report_pptx" / "generated_report.md"
    ...
    def test_summarize_doc_loads_saved_documents_and_writes_summary_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            summary_runs = list((root / "llm_output" / "document_pipeline").iterdir())
            ...
            self.assertIn("summary 3", workspace_path.read_text(encoding="utf-8"))
    ...
    def test_extract_single_doc_saves_file_scoped_artifacts(self) -> None:
        ...
        assert result is not None
        self.assertIn("llm_result/document_pipeline/report_pptx/documents/doc_report.json", result.text)
        self.assertIn("llm_result/document_pipeline/report_pptx/extraction_manifest.json", result.text)
    ...
    def test_workspace_status_reports_latest_summary_run_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            summary_root = root / "llm_output" / "document_pipeline"
        ...
        assert result is not None
        self.assertIn("llm_output/document_pipeline/notes_20240101_000001/document_summaries.json", result.text)
        self.assertIn("llm_output/document_pipeline/notes_20240101_000001/workspace_summary.md", result.text)
