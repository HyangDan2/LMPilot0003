...
* **Document pipeline slash tools**
  ...
  ```text
  <attached-folder>/llm_output/document_pipeline/<run-name>/
  ```
  ...
  `/summarize_doc` creates hierarchical LLM-backed summaries from extracted documents with bounded chunking and workspace-level synthesis. If a path is provided, it summarizes only that file from the attached folder.
  ...
  ```text
  <attached-folder>/llm_result/document_pipeline/<file-scope>/
  ```

  Examples:

  ```text
  llm_result/document_pipeline/design_review_pptx/extracted_documents.json
  llm_result/document_pipeline/design_review_pptx/document_map.json
  llm_result/document_pipeline/design_review_pptx/generated_report.md
  ```
  ...
  During normal chat, the model can request previously generated artifacts from the attached folder. The app safely executes these requests only under `<attached-folder>/llm_result/` and `<attached-folder>/llm_output/`, then sends the artifact content back to the model for a follow-up answer. Supported tags:
  ...
  Paths are resolved inside the attached folder and cannot escape `llm_result/` or `llm_output/`.
