...
For reasoning-model backends such as Qwen variants, the OpenAI-compatible client automatically detects `reasoning` / `reasoning_content` responses that contain no final assistant content. Streamed reasoning text is buffered internally and not shown in the chat body. The chat view shows only a minimal `Assistant: Processing...` placeholder while waiting for final content. If final assistant content never arrives, the raw backend JSON is pretty-printed directly for inspection, with no retry, no follow-up request, and no inferred final answer. If a normal assistant answer is present but the backend reports `finish_reason: "length"`, the client runs one hard-coded recovery procedure: it deterministically shortens the already-generated assistant text in code, keeps the most recent tail excerpt, and sends a single continuation request without any extra summary-generation call.
