...
* **Document pipeline slash tools**
  ...
  Reasoning-model backends that return `reasoning` or `reasoning_content` without final assistant content are handled automatically: the chat view shows only a minimal `Assistant: Processing...` placeholder while the response is being analyzed, and if no final assistant content arrives the raw backend JSON is pretty-printed for inspection without any retry or follow-up request.
...
For reasoning-model backends such as Qwen variants, the OpenAI-compatible client automatically detects `reasoning` / `reasoning_content` responses that contain no final assistant content. Streamed reasoning text is buffered internally and not shown in the chat body. The chat view shows only a minimal `Assistant: Processing...` placeholder while waiting for final content. If final assistant content never arrives and the backend did not report `finish_reason: "length"`, the raw backend JSON is pretty-printed directly for inspection, with no retry, no follow-up request, and no inferred final answer. If the backend reports `finish_reason: "length"`, the client treats that as the highest-priority recovery trigger even when final assistant `content` is missing. It runs one hard-coded recovery procedure: it deterministically shortens the available assistant-side text in code, keeps the most recent tail excerpt, and sends a single continuation request without any extra summary-generation call.
